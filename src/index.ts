...
const ruleFromStandard = (name: string): Linter.RuleEntry => {
  if (standardConfig.rules === undefined) throw new Error()
  const rule = standardConfig.rules[name]
  ...
  overrides: [
    ...
    },
    {
      ...
      rules: {
        ...
        ...fromEntries(
          equivalents.map((name) => [`@typescript-eslint/${name}`, ruleFromStandard(name)])
        ),
